...
export const verifierStatutEnvoi = async (numero, creatorId, maxTentatives = 5, intervalMs = 2000) => {
          ...
          const checkInterval = setInterval(async () => {
            ...
            try {
              ...
              const messageSentitems = await SentItems.findOne({
                ...
                order: [['ID', 'DESC']],
                attributes: ['Status', 'SendingDateTime', 'DeliveryDateTime', 'StatusError']
              });

              if (messageSentitems) {
                // Message trouvé dans sentitems = envoyé avec succès
                clearInterval(checkInterval);
                console.log(`✅ Message trouvé dans sentitems pour ${numero}, Status: ${messageSentitems.Status}`);
                resolve({ 
                  success: true, 
                  data: {
              ...
              console.log(`🔍 Tentative ${tentatives}/${maxTentatives} pour ${numero} - Dans outbox: ${!!messageOutbox}`);
              ...
              console.error(`❌ Erreur vérification statut ${numero}:`, error.message);
            }
          }, intervalMs);
        });
      };